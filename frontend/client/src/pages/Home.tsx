...
function TeamView() {
  ...
  const logout = () => { localStorage.removeItem("tracelock-session"); setUser(null); };
  return <section className="service-view"><div className="service-hero"><div><div className="eyebrow lime-text"><span className="signal-line" /> Team access</div><h1>Operators & roles</h1><p>Local team authentication protects scenario execution and incident-management actions.</p></div><div className="api-pill"><StatusDot /> Role-aware access</div></div><div className="panel settings-card"><span className="eyebrow">Local operator sign-in</span><h2>{user ? "Session active" : "Sign in to TraceLock"}</h2>{user ? <><pre>{JSON.stringify(user, null, 2)}</pre><button className="button button-quiet" onClick={logout}>Sign out</button></> : <><div className="settings-row"><input value={username} onChange={(event) => setUsername(event.target.value)} placeholder="username" /><input type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="password" /><button className="button button-primary" onClick={login}>Sign in</button></div>{error && <div className="service-error-copy"><CircleAlert size={18} /><p>{error}</p></div>}<div className="settings-note"><Info size={16} /><span>Local demo accounts: admin/admin-tracelock-local, operator/operator-tracelock-local, viewer/viewer-tracelock-local.</span></div></>}</div></section>;
}
