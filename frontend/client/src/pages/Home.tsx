...
function ScenarioView() {
  ...
  const run = (scenario: string) => {
    const token = localStorage.getItem("tracelock-session");
    if (!token) { setResult({ error: "Sign in from Team access before running a scenario." }); return; }
    setRunning(scenario);
    fetch(`${apiBase}/v1/demo/scenarios`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ scenario }) })
      .then(async (response) => { const body = await response.json(); if (!response.ok) throw new Error(body.detail || `Gateway returned HTTP ${response.status}`); setResult(body); })
      .catch((error) => setResult({ error: error instanceof Error ? error.message : "Scenario failed" }))
      .finally(() => setRunning(""));
  };
  return <section className="service-view"><div className="service-hero"><div><div className="eyebrow lime-text"><span className="signal-line" /> Safe operations lab</div><h1>Traffic scenarios</h1><p>Run bounded demonstrations through the real gateway path and immediately inspect the decision evidence.</p></div><div className="api-pill"><StatusDot /> No receiver outside the local transport</div></div><div className="admin-grid"><div className="panel admin-card"><span className="eyebrow">Demonstration controls</span><h2>Exercise enforcement</h2><p>These scenarios are explicit, local, and auditable. They never send data to an external receiver.</p><div className="service-actions"><button className="button button-primary" onClick={() => run("allow")} disabled={!!running}><Play size={15} /> {running === "allow" ? "Running…" : "Allow"}</button><button className="button button-quiet" onClick={() => run("redact")} disabled={!!running}><ShieldCheck size={15} /> Redact</button><button className="button button-quiet" onClick={() => run("block")} disabled={!!running}><CircleAlert size={15} /> Block</button><button className="button button-quiet" onClick={() => run("bypass")} disabled={!!running}><Network size={15} /> Bypass</button></div></div><div className="panel admin-card"><span className="eyebrow">Latest result</span><h2>Gateway decision</h2><pre>{result ? JSON.stringify(result, null, 2) : "Run a scenario to create a real evidence record."}</pre></div></div></section>;
}

function TeamView() {
  const [username, setUsername] = useState("operator");
  const [password, setPassword] = useState("operator-tracelock-local");
  const [user, setUser] = useState<unknown>(null);
  const [error, setError] = useState("");
  const [checking, setChecking] = useState(true);
  const clearSession = () => { localStorage.removeItem("tracelock-session"); setUser(null); };
  useEffect(() => {
    const token = localStorage.getItem("tracelock-session");
    if (!token) { setChecking(false); return; }
    fetch(`${apiBase}/v1/auth/me`, { headers: { Authorization: `Bearer ${token}` }, signal: AbortSignal.timeout(3000) })
      .then(async (response) => { const body = await response.json(); if (!response.ok) throw new Error(body.detail || "Session expired"); setUser(body); })
      .catch(() => { clearSession(); setError("Your session expired. Sign in again."); })
      .finally(() => setChecking(false));
  }, []);
  const login = () => { setError(""); fetch(`${apiBase}/v1/auth/login`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username, password }) }).then(async (response) => { const body = await response.json(); if (!response.ok) throw new Error(body.detail || "Login failed"); localStorage.setItem("tracelock-session", body.access_token); setUser(body.user); }).catch((reason) => setError(reason instanceof Error ? reason.message : "Login failed")); };
  const logout = () => { const token = localStorage.getItem("tracelock-session"); if (!token) { clearSession(); return; } fetch(`${apiBase}/v1/auth/logout`, { method: "POST", headers: { Authorization: `Bearer ${token}` } }).finally(clearSession); };
  return <section className="service-view"><div className="service-hero"><div><div className="eyebrow lime-text"><span className="signal-line" /> Team access</div><h1>Operators & roles</h1><p>Sign in to protect operator actions. Sessions persist across reloads and can be revoked with Sign out.</p></div><div className="api-pill"><StatusDot /> Role-aware access</div></div><div className="panel settings-card"><span className="eyebrow">Local operator sign-in</span><h2>{checking ? "Checking session…" : user ? "Session active" : "Sign in to TraceLock"}</h2>{user ? <><pre>{JSON.stringify(user, null, 2)}</pre><button className="button button-quiet" onClick={logout}>Sign out everywhere for this session</button></> : <><div className="settings-row"><input value={username} onChange={(event) => setUsername(event.target.value)} placeholder="username" /><input type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="password" /><button className="button button-primary" onClick={login} disabled={checking}>Sign in</button></div>{error && <div className="service-error-copy"><CircleAlert size={18} /><p>{error}</p></div>}<div className="settings-note"><Info size={16} /><span>Development accounts: admin/admin-tracelock-local, operator/operator-tracelock-local, viewer/viewer-tracelock-local.</span></div></>}</div></section>;
}
  ...
  return (
    <div className="app-shell">
      <aside className={`sidebar ${mobileNav ? "sidebar-open" : ""}`}>
        ...
        <nav><button className="nav-item" onClick={() => setActiveNav("Integrations")}><GitBranch size={17} /><span>Integrations</span></button><button className="nav-item" onClick={() => setActiveNav("Team access")}><Users size={17} /><span>Team access</span></button><button className="nav-item" onClick={() => setActiveNav("Settings")}><SlidersHorizontal size={17} /><span>Settings</span></button></nav>
